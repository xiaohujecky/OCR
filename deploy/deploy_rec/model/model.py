...
class Model(object):
    ...
    def __init__(self,
        ...
        gpu_device_id = ['/gpu:' + gpu for gpu in str(gpu_id).split(',')]
        ...
        if phase == 'train':
            ...
            assert False, phase
        for gpu_device in gpu_device_id:
    ...
    def launch(self):
        ...
        if self.phase == 'test':
            ...
            logging.info('train total size : %d'%total)
    ...
    def visualize_attention(self, filename, attentions, output_valid, ground_valid, flag_incorrect, real_len):
        ...
        output_dir = os.path.join(output_dir, filename.replace('/', '_')[1:])
        ...
        with open(os.path.join(output_dir, 'word.txt'), 'w') as fword:
            ...
            print(output_valid)
            fword.write(' '.join([alphabet2lex_map[c.astype(np.unicode_).encode('utf-8')] for c in ground_valid])+'\n')
            fword.write(' '.join([alphabet2lex_map[c.astype(np.unicode_).encode('utf-8')] for c in output_valid]))
            fword.write('\n')
